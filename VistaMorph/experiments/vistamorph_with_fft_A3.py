...
def sample_images(batches_done):
    ...
    warped_B = model(img_A=sp_A, img_B=sp_B, src=real_B)
    fake_A = generator2(warped_B)

    img_sample_global = torch.cat((real_A.data, sp_A.data, real_B.data, warped_B.data, fake_A.data, fake_B.data), -1)
...
for epoch in range(opt.epoch, opt.n_epochs):
    for i, batch in enumerate(dataloader):
        ...
        with autocast():
            ...
            # pass to generator 2 for fake_A
            warped_B, total_theta = model(img_A=real_A, img_B=sp_B, src=real_B)
            fake_A = generator2(warped_B)
            ...
            loss_G = (loss_GAN + recon_loss + perc_loss).mean()
