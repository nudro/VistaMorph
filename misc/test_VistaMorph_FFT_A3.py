...
def speckle(image):
    """Add Rayleigh distributed speckle noise to the image."""
    ...
    speckled = img_float * noise
    
    # Clip and convert back to original type
    speckled = torch.clamp(speckled * 255, 0, 255).type(image.dtype)
    return speckled
...
for i, batch in tqdm(enumerate(test_dataloader)):
    ...
    with torch.no_grad(): 
        ...
        fake_B = generator1(sp_A)

        # pass to generator 2 for fake_A
        warped_B, theta = net(img_A=sp_A, img_B=sp_B, src=real_B)
    ...
    # GLOBAL
    #img_sample_global = torch.cat((real_A.data, real_B.data, real_A.data, warped_B.data, fake_A.data, fake_B.data), -1)
    #save_image(img_sample_global, "./images/test_results/%s/%s.png" % (opt.experiment, i), nrow=1, normalize=True)
    ...
    img_sample = torch.cat((
        real_A_denorm, real_B_denorm, real_A_denorm, warped_B_denorm, fake_A.data, fake_B.data), -1)
