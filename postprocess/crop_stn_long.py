...
"""
Crops a 1280 x 256 images of stacked images from the test phase:
real_A, fake_B, real_B and puts them into respective directories. Run
this before evaluation.py

STN experiment outputs 1280x256 stack of:
...
def crop_it(infile_path, RA_out, RegB_out, RB_out):
    ...
    for item in dirs:
        ...
        if os.path.isfile(fullpath):
            ...
            # For 1280x256 images, we need to crop horizontally
            # Each image is 256x256, so we need to adjust the x coordinates
            real_A = im.crop((0, 0, 256, 256))
            real_B = im.crop((256, 0, 512, 256))
            fake_B = im.crop((512, 0, 768, 256))
            fake_wB = im.crop((768, 0, 1024, 256))
            reg_B = im.crop((1024, 0, 1280, 256))
